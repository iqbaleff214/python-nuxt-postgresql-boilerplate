...
async function handleChangePassword() {
  ...
  if (!passwordForm.current_password) {
    passwordErrors.current_password = t('security.currentPasswordRequired')
    valid = false
  }
  if (passwordForm.new_password.length < 8) {
    passwordErrors.new_password = t('security.newPasswordMin')
    valid = false
  }
  if (passwordForm.new_password !== passwordForm.confirm_password) {
    passwordErrors.confirm_password = t('validation.passwordsNoMatch')
    valid = false
  }
  if (!valid) return
  ...
  if (response.success) {
    ...
    passwordErrors.current_password = response.message || t('security.passwordChangeFailed')
  }
...
async function handleDisable2fa() {
  if (!disable2faPassword.value) {
    toast.error(t('security.enterPasswordToConfirm'))
    return
  }
  if (disable2faOtp.value.length !== 6) {
    disable2faOtpError.value = t('security.enter6DigitCode')
    return
  }
  ...
  if (response.success) {
    ...
    toast.error(response.message || t('security.disable2FAFailed'))
  }
...
const strengthLabel = computed(() => {
  ...
  return labels[newPasswordStrength.value] ?? ''
})
...
<template>
  <div class="space-y-6">
    <!-- Header -->
    <div>
      <h1 class="text-2xl font-bold text-slate-900">{{ $t('security.title') }}</h1>
      <p class="mt-0.5 text-sm text-slate-500">{{ $t('security.subtitle') }}</p>
    </div>

    <!-- Change Password -->
    <AppCard :title="$t('security.changePassword')" :subtitle="$t('security.changePasswordSub')">
    ...
    <AppCard :title="$t('security.twoFA')" :subtitle="$t('security.twoFASub')">
        ...
        <div class="flex items-center gap-3 rounded-lg bg-emerald-50 border border-emerald-200 p-4">
          ...
          <div class="flex-1">
            <div class="flex items-center gap-2">
              <p class="font-semibold text-emerald-800">{{ $t('security.twoFAActive') }}</p>
              <AppBadge variant="success">{{ $t('security.enabled') }}</AppBadge>
            </div>
            <p class="mt-0.5 text-sm text-emerald-700">{{ $t('security.twoFAActiveMsg') }}</p>
          </div>
        </div>
      ...
      <div v-else class="space-y-4">
        <div class="flex items-start gap-3 rounded-lg bg-amber-50 border border-amber-200 p-4 mb-4">
          ...
          <div>
            <p class="font-semibold text-amber-800">{{ $t('security.twoFANotEnabled') }}</p>
            <p class="mt-0.5 text-sm text-amber-700">{{ $t('security.twoFANotEnabledMsg') }}</p>
          </div>
        </div>
    ...
    <!-- Disable 2FA modal -->
    <AppModal v-model="showDisable2faModal" :title="$t('security.disableTitle')" size="sm">
      <div class="space-y-4">
      ...
      <template #footer>
        <div class="flex gap-3 justify-end">
          <AppButton variant="secondary" @click="showDisable2faModal = false; disable2faOtpError = ''">{{ $t('common.cancel') }}</AppButton>
